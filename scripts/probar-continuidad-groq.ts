#!/usr/bin/env tsx
// Prueba de CONTINUIDAD de conexión contra Groq: ¿el 21% de "Connection error"
// visto en probar-latencia-groq.ts es ruido aleatorio uniforme por llamada, o
// se concentra al inicio de la sesión (cold-start de la conexión TCP/TLS)?
//
// Metodología: UN SOLO cliente OpenAI (como hace clienteLLM() en producción,
// que se crea una vez por proceso y se reutiliza) hace ~42 llamadas seguidas
// contra openai/gpt-oss-120b, repitiendo los mismos 14 prompts de la prueba
// anterior 3 veces. Se registra la POSICIÓN de cada llamada en la secuencia
// para poder ver si los fallos se agrupan al principio o se reparten parejo.
//
// Uso: npx tsx scripts/probar-continuidad-groq.ts
